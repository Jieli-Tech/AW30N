...
void usb_hid_set_second_repport_map(const u8 *map, int size)
{
...
static u32 hid_tx_data(struct usb_device_t *usb_device, const u8 *buffer, u32 len)
{
    ...
    return usb_g_intr_write(usb_id, HID_EP_IN_2, buffer, len);
}
...
{
    const usb_dev usb_id = usb_device2id(usb_device);
    usb_g_ep_config(usb_id, HID_EP_IN_2 | USB_DIR_IN, USB_ENDPOINT_XFER_INT, 0, hid_ep_in_dma, MAXP_SIZE_HIDIN);
    usb_enable_ep(usb_id, HID_EP_IN_2);
...
u32 hid_register_second(const usb_dev usb_id)
{
    hid_ep_in_dma = usb_alloc_ep_dmabuffer(usb_id, HID_EP_IN_2 | USB_DIR_IN, MAXP_SIZE_HIDIN);

    /* hid_ep_out_dma = usb_alloc_ep_dmabuffer(usb_id, HID_EP_OUT,MAXP_SIZE_HIDOUT); */
    return 0;
}

void hid_release_second(const usb_dev usb_id)
{
    return ;
}
...
#if USB_ROOT2
    usb_disable_ep(usb_id, HID_EP_IN_2);
#else
...
u32 hid_send_second_data(const void *p, u32 len)
{
    if (!hid_device_hdl) {
        return 0;
    }
