...
struct hid_device_var_t _custom_hid_var AT(.hid_config_var);
struct hid_device_var_t *custom_hid_var AT(.usb_hid.keep_ram);

void custom_hid_init(void)
{
    memset((void *)&_custom_hid_var, 0, sizeof(_custom_hid_var));
    _custom_hid_var.ep_in_buffer = usb_get_ep_buffer(0, CUSTOM_HID_EP_IN | USB_DIR_IN);
    _custom_hid_var.ep_out_buffer = usb_get_ep_buffer(0, CUSTOM_HID_EP_OUT | USB_DIR_OUT);
}
