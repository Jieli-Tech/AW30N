...
u32 sbc_encode_api(void *p_file, void *input_func, void *output_func)
{
    ...
    sbc_para_set.msbc = 0;		//0：编码sbc ； 1：编码msbc
    if (sbc_para_set.msbc == 0) {		//选择sbc时
        ...
        for (u8 i = 0; i < sizeof(sbc_sr_tab); i++) {
            ...
            if (i == sizeof(sbc_sr_tab)) {		//sbc时，采样率都不匹配时return退出
                log_error("sbc sr not match %d\n", sbc_para_set.sr);
                return 0;
            }
        }
    } else if (sbc_para_set.msbc == 1) {
        /* 选择msbc时，参数库内固定 */
