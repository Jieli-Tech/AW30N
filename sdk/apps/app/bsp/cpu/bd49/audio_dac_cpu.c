...
#define DAC_MAX_SP  (sizeof(audio_dac_buf) / (AUDAC_TRACK_NUMBER * (AUDAC_BIT_WIDE / 8)))
...
#define A_DAC_FADE   0
#else
#define A_DAC_FADE   A1DAC_FADE_EN
#endif
...
_OTP_CONST_  DAC_CTRL_HDL audio_dac_ops = {
    ...
    .con1        = DAC_CON1_DEFAULT | A_DAC_FADE | A1DAC_FADE_SLOW(TCFG_DAC_VOL_FADE_SLOW) | A1DAC_FADE_STEP(TCFG_DAC_VOL_FADE_STEP),
    .pns         = DAC_PNS,
};
...
#else
    SFR(JL_AUDIO->DAC_DIT,  0, 16, -1024);
    SFR(JL_AUDIO->DAC_TM0,  0, 16, 0);
...
{
    log_info("audac_init");
    dac_phy_init(audac_sr_lookup(sr));
    if (delay_flag) {
        udelay(2000);
    }
    dac_trim_api();
