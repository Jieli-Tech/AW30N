...
static void __slv_reconnect_low_timeout_handle(void)
{
    if (0 == con_handle) {
...
static void advertisements_setup_init()
{
    ...
    if (PAIR_RECONNECT_ADV_EN && slv_pair_info.pair_flag && slv_pair_info.direct_adv_cnt) {
        ...
        if (pair_reconnect_adv_timeout) {
            ...
            slv_adv_timeout_number = sys_timeout_add(0, __slv_reconnect_low_timeout_handle, pair_reconnect_adv_timeout);
