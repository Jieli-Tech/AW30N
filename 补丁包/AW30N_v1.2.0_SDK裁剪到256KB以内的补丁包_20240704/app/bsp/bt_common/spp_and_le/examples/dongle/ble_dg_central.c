    ...
    case GATT_COMM_EVENT_GATT_DATA_REPORT: {
        ...
        case GATT_EVENT_NOTIFICATION: { //notify
            if (report_data->value_handle != ota_notify_handle[0]) {
                ...
                case ATT_CHARACTERISTIC_2a4d_06_VALUE_HANDLE:
                case ATT_CHARACTERISTIC_2a4d_07_VALUE_HANDLE:
...
void bt_ble_init(void)
{
    ...
    if (CONFIG_BT_GATT_CLIENT_NUM == 1) {
        ...
        dg_timer_handle(0);
#endif
        dg_timer_id = sys_timeout_add((void *)1, dg_timer_handle, POWER_ON_SWITCH_TIME);
    } else {
        ble_gatt_client_scan_enable(1);
        log_info("just scan");
    }
