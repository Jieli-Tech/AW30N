...
int ll_vendor_rx_max(uint16_t conn_handle, uint16_t rxmax);
...
void ll_padvb_vendor_push_data(u8 handle, uint8_t *push_data, uint16_t data_len);
void ll_ext_padvb_up_data_callback_register(void (*callback)());
void ll_ext_padvb_pop_data_callback_register(void (*callback)(uint8_t *data));
void ll_ext_set_auxoffset(uint32_t offset);
void ll_padv_set_syncoffset(uint32_t offset);
void ll_hci_set_ext_scan_params(u8 *data, u32 size);
void ll_hci_set_ext_scan_enable(u8 *data, u32 size);
void ll_hci_periodic_adv_creat_sync(u8 *data, u32 size);
void ll_hci_periodic_adv_terminate_sync(u8 *data, u32 size);
...
void ll_padv_get_sync_frame_interval(uint32_t interval);
void ll_padv_get_user_data_len(uint8_t len);
int le_periodic_advertising_create_sync(const u8 *data, u32 size);
int le_periodic_advertising_terminate_sync(const u8 *data, u32 size);
int le_set_extended_scan_enable(uint8_t enable, uint8_t filter, uint16_t duration, uint16_t period);
void ll_padv_get_user_flow_mode(uint8_t mode);
void ll_vendor_set_code_type(u8 code_type);
void ll_periodic_advertising_create_sync_cancel(void);
void ll_vendor_pri_reset_conn();
