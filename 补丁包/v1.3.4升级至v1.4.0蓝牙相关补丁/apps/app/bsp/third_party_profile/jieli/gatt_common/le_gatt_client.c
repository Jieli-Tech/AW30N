 ...
 */
/*************************************************************************************************/
static void __gatt_client_timeout_handler(void *type_id)
{
...
void user_client_report_data_callback(att_data_report_t *report_data)
{
    ...
    __gatt_client_event_callback_handler(GATT_COMM_EVENT_GATT_DATA_REPORT, (u8 *)report_data, sizeof(att_data_report_t), 0);
}
    ...
    for (i = 0; i < __this->opt_handle_used_cnt; i++) {
        ...
        if (cur_opt_type & ATT_PROPERTY_NOTIFY) {
            if (__this->gatt_search_config->auto_enable_ccc) {
                ...
                ble_comm_att_send_data(__this->client_search_handle, opt_hdl_pt->value_handle + 1, (u8 *)&tmp_16, 2, ATT_OP_WRITE);
                if (0x2a4d == opt_hdl_pt->search_uuid->characteristic_uuid16 && opt_hdl_pt->search_uuid->read_report_reference) {
                    ...
                    ble_comm_att_send_data(__this->client_search_handle, opt_hdl_pt->value_handle + 1, (u8 *)&tmp_16, 2, ATT_OP_READ);
                    log_info("read_desc02:%04x\n", opt_hdl_pt->value_handle + 2);
                    ble_comm_att_send_data(__this->client_search_handle, opt_hdl_pt->value_handle + 2, (u8 *)&tmp_16, 2, ATT_OP_READ);
                }
            }
            ...
            if (opt_hdl_pt->search_uuid->read_long_enable) {
                ...
                ble_comm_att_send_data(__this->client_search_handle, opt_hdl_pt->value_handle, (u8 *)&tmp_16, 2, ATT_OP_READ_LONG);
            } else {
                tmp_16  = 0x55A1;//fixed
                log_info("read:%04x\n", opt_hdl_pt->value_handle);
                ble_comm_att_send_data(__this->client_search_handle, opt_hdl_pt->value_handle, (u8 *)&tmp_16, 2, ATT_OP_READ);
            }
        } else {
            ;
        }
...
static void __check_target_uuid_match(search_result_t *result_info)
{
    ...
    for (i = 0; i < __this->gatt_search_config->search_uuid_count; i++) {
        t_uuid = (target_uuid_t *)&__this->gatt_search_config->search_uuid_group[i];
    ...
    __gatt_client_event_callback_handler(GATT_COMM_EVENT_GATT_SEARCH_MATCH_UUID, (u8 *)opt_get, sizeof(opt_handle_t), 0);
...
void user_client_report_descriptor_result(charact_descriptor_t *result_descriptor)
{
    ...
    __gatt_client_event_callback_handler(GATT_COMM_EVENT_GATT_SEARCH_DESCRIPTOR_RESULT, (u8 *)&result_descriptor->handle, 2, (u8 *)result_descriptor);
}
...
void user_client_report_search_result(search_result_t *result_info)
{
    if (result_info == (void *) - 1) {
        ...
        __gatt_client_event_callback_handler(GATT_COMM_EVENT_GATT_SEARCH_PROFILE_COMPLETE, (u8 *)&__this->client_search_handle, 2, 0);
...
static void __gatt_client_search_profile_start(void)
{
    ...
    __gatt_client_event_callback_handler(GATT_COMM_EVENT_GATT_SEARCH_PROFILE_START, (u8 *)&__this->client_search_handle, 2, 0);
...
static bool __check_device_is_match(u8 event_type, u8 info_type, u8 *data, int size, client_match_cfg_t **output_match_devices)
{
    ...
    for (i = 0; i < __this->gatt_search_config->match_devices_count; i++) {
        cfg = (client_match_cfg_t *)&__this->gatt_search_config->match_devices[i];
        if (cfg == NULL) {
            continue;
        }
...
#else
    /*全参数格式*/
    struct create_conn_param_ext_t *create_conn_par = (struct create_conn_param_ext_t *)scan_buffer;
...
static void __gatt_client_report_adv_data(adv_report_t *report_pt, u16 len)
{
    ...
    if (!__this->gatt_search_config || !__this->gatt_search_config->match_devices_count) {
        ...
        putchar('~');
        __gatt_client_event_callback_handler(GATT_COMM_EVENT_SCAN_ADV_REPORT, (u8 *)report_pt, len, 0);
        return;
    }
...
void ble_gatt_client_passkey_input(u32 *key, u16 conn_handle)
{
    ...
    *key = 123456; //default key
    ...
    extern void little_endian_store_32(uint8_t *buffer, uint16_t pos, uint32_t value);
    little_endian_store_32((u8 *)tmp_val, 2, (u32)key);
    __gatt_client_event_callback_handler(GATT_COMM_EVENT_SM_PASSKEY_INPUT, (u8 *)tmp_val, 6, 0);
...
void ble_gatt_client_sm_packet(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size)
{
    ...
    case HCI_EVENT_PACKET:
        ...
        case SM_EVENT_JUST_WORKS_REQUEST:
            ...
            __gatt_client_event_callback_handler(GATT_COMM_EVENT_ENCRYPTION_REQUEST, (u8 *)&event->con_handle, 2, (u8 *)&__this->client_encrypt_process);
            break;
...
void ble_gatt_client_cbk_packet_handler(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size)
{
    ...
    case HCI_EVENT_PACKET:
        ...
        case HCI_EVENT_DISCONNECTION_COMPLETE: {
            ...
            __gatt_client_event_callback_handler(GATT_COMM_EVENT_DISCONNECT_COMPLETE, (u8 *)tmp_val, 4, packet);
            __gatt_client_check_auto_scan();
        }
        break;
        ...
        case ATT_EVENT_MTU_EXCHANGE_COMPLETE: {
            ...
            __gatt_client_event_callback_handler(GATT_COMM_EVENT_MTU_EXCHANGE_COMPLETE, (u8 *)tmp_val, 4, 0);
        }
        break;
