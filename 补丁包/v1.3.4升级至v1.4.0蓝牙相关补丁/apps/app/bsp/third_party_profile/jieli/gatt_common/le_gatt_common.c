...
extern u8 sm_return_master_reconn_encryption();
//----------------------------------------------------------------------------------------
/*************************************************************************************************/
/*!
...
#if CONFIG_BT_GATT_COMMON_GET_ADV_TEPORT_ENABLE && CONFIG_HOGP_COMMON_ENABLE
            extern void get_gap_event_advertising_report(adv_report_t *report_pt, u16 len);
            get_gap_event_advertising_report((void *)&packet[2], packet[1]);
#endif
#endif
            break;
...
void ble_profile_init(void)
{
    ...
    if (STACK_IS_SUPPORT_SM_PAIR()) {
        if (gatt_control_block->sm_config) {
            ...
            u8 info = gatt_control_block->sm_config->master_security_auto_req;
            sm_set_master_request_pair(&info);
    ...
    if (STACK_IS_SUPPORT_GATT_CONNECT()) {
        ...
        log_info("mtu_size = %d, cbuf_size = %d\n", gatt_control_block->mtu_size, gatt_control_block->cbuffer_size);
        log_info("need_ram = %d\n", need_ram);
        if (!gatt_ram_buffer) {
            extern void *__bt_malloc(int size);
            gatt_ram_buffer = __bt_malloc(need_ram);
...
void ble_comm_init(const gatt_ctrl_t *control_blk)
{
    ...
    gatt_control_block = (gatt_ctrl_t *)control_blk;
...
{
    ble_cmd_ret_e ret = BLE_CMD_RET_SUCESS;
    u16 tmp_16;
