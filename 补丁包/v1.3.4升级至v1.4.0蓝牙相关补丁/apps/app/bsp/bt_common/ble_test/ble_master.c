...
#define SUPPORT_TEST_BOX_BLE_MASTER_TEST_EN_M	1
...
static void (*master_app_update_parm_succ_callback)(int parm) = NULL;
...
//指定搜索uuid
//指定搜索uuid
static target_uuid_t  default_search_uuid_table[] = {
...
{
    log_info("%s\n", __FUNCTION__);
    client_operation_send(notify_handle[0] + 1, (u8 *)&ccc_value, 2, ATT_OP_WRITE);
    client_operation_send(notify_handle[1] + 1, (u8 *)&ccc_value, 2, ATT_OP_WRITE);
    client_operation_send(notify_handle[2] + 1, (u8 *)&ccc_value, 2, ATT_OP_WRITE);

    //RCSP
    client_operation_send(ATT_SEND_DATA_RCSP, (u8 *)&ccc_value, 2, ATT_OP_WRITE);
}
...
{
    u32 pac_info = (u32)packet;
    if ((pac_info & 0x3) == 0) {
        /* log_error("pack_no_al"); */
    }

    switch (event) {
    case CLI_EVENT_MATCH_DEV: {
        client_match_cfg_t *match_dev = (client_match_cfg_t *)pac_info;
        log_info("match_name:%s\n", match_dev->compare_data);
    }
    break;

    case CLI_EVENT_MATCH_UUID: {
        opt_handle_t *opt_hdl = (opt_handle_t *)pac_info;
...
static bool resolve_adv_report(u8 *adv_address, u8 data_length, u8 *data, s8 rssi)
{
    ...
    for (i = 0; i < data_length;) {
        ...
        case HCI_EIR_DATATYPE_SHORTENED_LOCAL_NAME:
            ...
            //-------
#if SUPPORT_TEST_BOX_BLE_MASTER_TEST_EN_M
...
static void connection_update_complete_success(u8 *packet)
{
    ...
    if (NULL != master_app_update_parm_succ_callback) {
        master_app_update_parm_succ_callback(conn_interval);
    }
    log_info("master update_complete_succ:\n");
...
static void cbk_packet_handler(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size)
{
    ...
    case HCI_EVENT_PACKET:
        ...
        case HCI_EVENT_LE_META:
            ...
            case HCI_SUBEVENT_LE_CONNECTION_UPDATE_COMPLETE:
                ...
                if (!config_le_sm_support_enable) {
                    if (config_btctler_le_features & LE_DATA_PACKET_LENGTH_EXTENSION) {
                        ...
                        set_connection_data_phy(CONN_SET_2M_PHY, CONN_SET_2M_PHY, CONN_SET_PHY_OPTIONS_NONE);
                    } else if (config_btctler_le_features & LE_CODED_PHY) {
                        if (config_btctler_coded_type == CONN_SET_PHY_OPTIONS_S2) {
                            log_info(">>>>>>>>s1--request CODED S2\n");
                            set_connection_data_phy(CONN_SET_CODED_PHY, CONN_SET_CODED_PHY, CONN_SET_PHY_OPTIONS_S2);
                        }
                    }
                }
                break;
            ...
            case HCI_SUBEVENT_LE_DATA_LENGTH_CHANGE:
                ...
                if (config_btctler_le_features & LE_2M_PHY) {
                    log_info(">>>>>>>>s3--request 2M\n");
                    set_connection_data_phy(CONN_SET_2M_PHY, CONN_SET_2M_PHY, CONN_SET_PHY_OPTIONS_NONE);
                } else if (config_btctler_le_features & LE_CODED_PHY) {
                    if (config_btctler_coded_type == CONN_SET_PHY_OPTIONS_S2) {
                        log_info(">>>>>>>>s3--request CODED S2\n");
                        set_connection_data_phy(CONN_SET_CODED_PHY, CONN_SET_CODED_PHY, CONN_SET_PHY_OPTIONS_S2);
                    }
                }
                break;
        ...
        case HCI_EVENT_DISCONNECTION_COMPLETE:
            ...
            //auto to do
            if (conn_pair_info.pair_flag && scan_ctrl_en) {
        ...
        case HCI_EVENT_ENCRYPTION_CHANGE:
            ...
            if (0 == client_config->search_uuid_cnt) {
                enable_notify_ccc(con_handle);
            }

            if (config_le_sm_support_enable) {
                if (config_btctler_le_features & LE_DATA_PACKET_LENGTH_EXTENSION) {
                    log_info(">>>>>>>>s2--request DLE\n");
                    set_connection_data_length(ATT_LOCAL_MTU_SIZE, 2120);
                } else if (config_btctler_le_features & LE_2M_PHY) {
                    log_info(">>>>>>>>s2--request 2M\n");
                    set_connection_data_phy(CONN_SET_2M_PHY, CONN_SET_2M_PHY, CONN_SET_PHY_OPTIONS_NONE);
                } else if (config_btctler_le_features & LE_CODED_PHY) {
                    if (config_btctler_coded_type == CONN_SET_PHY_OPTIONS_S2) {
                        log_info(">>>>>>>>s2--request CODED S2\n");
                        set_connection_data_phy(CONN_SET_CODED_PHY, CONN_SET_CODED_PHY, CONN_SET_PHY_OPTIONS_S2);
                    }
                }
            }
...
static int client_regiest_update_parm_succ_cbk(void *cbk)
{
    master_app_update_parm_succ_callback = cbk;
    return APP_BLE_NO_ERROR;
}
...
//该接口重新配置搜索的配置项
static int client_init_config(void *priv, void *__cfg)
{
    client_conn_cfg_t *cfg = (client_conn_cfg_t *)__cfg;
...
{
    return (struct ble_client_operation_t *)&client_operation;
}
...
//协议栈内部调用
extern void sm_set_master_request_pair(void *p);
...
int get_ble_master_connect_handle(void)
{
...
void bt_ble_master_update_parm_succ_register(void (*callback_func)(int parm))
{
    client_regiest_update_parm_succ_cbk((void *)callback_func);
}
...
const vble_smpl_role_ops ble_master_ops = {
    ...
    .conn_handle = get_ble_master_connect_handle,
    .update_conn_interval = bt_ble_master_update_conn_parm,
    .regist_update_parm_succ = bt_ble_master_update_parm_succ_register,
};
#endif
#endif
