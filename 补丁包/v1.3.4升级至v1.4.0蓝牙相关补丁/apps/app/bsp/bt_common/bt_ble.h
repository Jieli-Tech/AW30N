...
//BLE 性能测试 开关二选一 （DUT测试）---仅在TESTE_BLE_EN模式
#define BLE_DUT_TEST  0     // DUT测试模式,默认上电进初始化
#define BLE_DUT_API_TEST  0 // DUT api 模式,需要自行调用api测试,见ble_test_api.c
...
#define  SET_BLE_TX_POWER_LEVEL         (6)//0dBm
...
typedef struct _vble_smpl_role_ops {
    ...
    int (*conn_handle)(void);
    u32(*update_conn_interval)(u16 interval, u16 set_timeout);
    void (*regist_update_parm_succ)(void (*cb_func)(int parm));
} vble_smpl_role_ops;
