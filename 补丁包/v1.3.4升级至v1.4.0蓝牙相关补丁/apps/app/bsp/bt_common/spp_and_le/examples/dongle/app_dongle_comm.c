...
static int dongle_event_handler(struct application *app, struct sys_event *event)
{
    ...
    case SYS_BT_EVENT:
        ...
        if ((u32)event->arg == SYS_BT_EVENT_TYPE_CON_STATUS) {
            dongle_bt_connction_status_event_handler((struct bt_event *)&event->u.bt);
        } else if ((u32)event->arg == SYS_BT_EVENT_BLE_STATUS) {
            dongle_bt_ble_status_callback(event->u.bt.event, event->u.bt.value);
        } else if ((u32)event->arg == SYS_BT_EVENT_TYPE_HCI_STATUS) {
            dongle_bt_hci_event_handler((struct bt_event *)&event->u.bt);
#if RCSP_BTMATE_EN
        } else if ((u32)event->arg == DEVICE_EVENT_FROM_PC) {
            dongle_pc_event_handler((struct dg_ota_event *)&event->u.bt);//dongle pc命令回调处理
        } else if ((u32)event->arg == DEVICE_EVENT_FROM_OTG) {
            dongle_otg_event_handler((struct dg_ota_event *)&event->u.bt);//dongle ota升级数据透传
#endif
        }
#endif
        return 0;
