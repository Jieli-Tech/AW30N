...
// *****************************************************************************
//扫描接收到的数据
static void scan_rx_data_handle(const u8 *data, u16 len)
{
...
void le_advertising_report_event_upload(u8 subevent_code, u8 event_type, u8 addr_type, u8 *addr, int len, u8 *data, u8 rssi)
{
    ...
    for (i = 0; i < len;) {
        ...
        case HCI_EIR_DATATYPE_SHORTENED_LOCAL_NAME:
            ...
            /*log_info("unsigned rssi:%d\n", (u8 )rssi);*/
            if (!strcmp(data, "TEST2")) {
...
void bt_ble_init(void)
{
    ...
    ble_rf_vendor_fixed_channel(CFG_RF_ADV_SCAN_CHL, 3);  //通信信道，发送接收都要一致
#endif
