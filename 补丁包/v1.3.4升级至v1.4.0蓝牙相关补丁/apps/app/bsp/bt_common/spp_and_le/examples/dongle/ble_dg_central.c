...
#if defined (UPDATE_V2_EN) && (UPDATE_V2_EN)
...
#if (TRANS_DATA_SPPLE_EN && CONFIG_APP_DONGLE && TCFG_USER_BLE_ENABLE)
...
/* NOT_KEEP_RAM */
/* u8 gatt_ram_buffer[ATT_SEND_CBUF_SIZE + ATT_LOCAL_MTU_SIZE] __attribute__((aligned(4))); */
...
static u16 dg_ble_central_read_handle;
...
extern void ble_comm_module_enable(u8 en);
extern void ble_comm_exit(void);
extern int get_reonn_param(void);
extern void dongle_return_online_list(void *p);
extern void ble_comm_set_config_name(const char *name_p, u8 add_ext_name);
extern void dongle_send_data_to_pc_2(u16 con_handle, u8 *data, u16 len);
...
void check_is_reconn_succ(void *p);
...
int (*dongle_audio_input_handler)(u8 *packet, u16 size);
//------------------------------------------------------
//输入passkey 加密
#define PASSKEY_ENABLE                     0

static sm_cfg_t dg_sm_init_config = {
...
static gatt_client_cfg_t central_client_init_cfg;
...
static u16 conn_handle_local = 0;
...
static const u8 dg_test_remoter_name1[] = "Lenovo Go Multi-Device Mouse";//键盘
/* static const u8 dg_test_remoter_name2[] = "AC897N_MX(BLE)";//鼠标 */
static const u8 dg_test_remoter_name2[] = "JL_HID_SLAVE(BLE)";//鼠标
static const u8 user_config_tag_string[] = "abc123";
static client_match_cfg_t dg_match_device_table[] = {
    {
...
//input priv
static void dg_timer_handle(void *p)
{
...
#if RCSP_BTMATE_EN
    ble_comm_att_send_data(conn_handle, ota_notify_handle[0] + 1, (u8 *)&mouse_ccc_value, 2, ATT_OP_WRITE);
#endif
}
...
//dongle——hid 数据发送接口
int ble_dongle_send_data_ext(u8 *data, u16 len)
{
    //对应ble_hogp_profile.h中的input——最后一位att_handle
    dg_ble_central_read_handle = 0x003e;

    if (dg_conn_handle && dg_ble_central_read_handle) {
        return ble_comm_att_send_data(dg_conn_handle, dg_ble_central_read_handle, data, len, ATT_OP_WRITE);
    }
    return APP_BLE_OPERATION_ERROR;
}

int ble_master_data_send(u16 att_handle, u8 *data, u16 len)
{
    if (dg_conn_handle) {
        return ble_comm_att_send_data(dg_conn_handle, att_handle, data, len, ATT_OP_WRITE_WITHOUT_RESPOND);
    } else {
        return APP_BLE_OPERATION_ERROR;
    }
}
...
int ble_hid_data_send_ext(u8 report_type, u8 report_id, u8 *data, u16 len)
{
    ...
    dg_ble_central_write_handle = 0x002f;
...
int ble_dongle_send_data(u16 con_handle, u8 *data, u16 len)
{
    ...
    if (con_handle) {
        ...
        if (!record_bond_info[i].write_handle) {
...
static int dg_central_event_packet_handler(int event, u8 *packet, u16 size, u8 *ext_param)
{
    ...
    case GATT_COMM_EVENT_GATT_DATA_REPORT: {
        /* log_info("dg_central_event_packet_handler: %x\n", report_data->value_handle); */
        ...
        case GATT_EVENT_NOTIFICATION: { //notify
            if (report_data->value_handle != ota_notify_handle[0]) {
                ...
                case ATT_CHARACTERISTIC_HID_2a4d_07_VALUE_HANDLE:
                    vble_master_receive(report_data->value_handle, report_data->blob, report_data->blob_length);
                    break;
#if 0
                    if (dongle_input_handler) {
                        dongle_input_handler(report_data->blob, report_data->blob_length);
                    }
                    break;
                case ATT_CHARACTERISTIC_HID_2a4d_07_VALUE_HANDLE:
                    if (dongle_audio_input_handler) {
                        dongle_audio_input_handler(report_data->blob, report_data->blob_length);
                    }
                    break;
#endif
                default:
                ...
                int ret = dongle_input_handler(report_data->blob, report_data->blob_length);
#endif
#endif
            } else {
#if RCSP_BTMATE_EN
...
#if RCSP_BTMATE_EN
        u8 tmp_addr = 1;
        check_is_reconn_succ(&tmp_addr);//返回重连接成功,未放到encry处,避免不走加密情况
#endif
        break;
    ...
    case GATT_COMM_EVENT_DISCONNECT_COMPLETE: {
        ...
        if (app_get_jl_updating_flag()) {
            if (!get_reonn_param()) {
                dongle_return_online_list(NULL);
            }
            if (reonn_channel_is_change()) {
                dongle_ota_init();
            }
            extern void clear_auth(u16 channel);
            void judge_is_reonn(void *p);
            ...
            conn_handle_local = little_endian_read_16(packet, 0);
            sys_timeout_add((void *)&conn_handle_local, judge_is_reonn, 1000);//判断是否是ota回连接
        }
#endif
    }
    break;
    ...
    case GATT_COMM_EVENT_ENCRYPTION_CHANGE:
        ...
        __ble_state_to_user(BLE_PRIV_PAIR_ENCRYPTION_CHANGE, little_endian_read_16(packet, 0));
        ...
        for (u8 i = 0; i < SUPPORT_MAX_GATT_CLIENT; i++) {
            if (record_bond_info[i].write_handle) {
                log_info("open ota :%d, ota handle:%d", i, record_bond_info[i].write_handle);
    ...
    case GATT_COMM_EVENT_SCAN_DEV_MATCH: {
        ...
        if (packet[9] < SUPPORT_MAX_GATT_CLIENT) {
            /*记录表地址回连，使用记录的连接参数建立*/
            printf("match bond,reconnect\n");
            dg_scan_conn_config_set(&record_bond_info[packet[9]]);
        } else {
            /*搜索匹配方式连接*/
            printf("match search_config\n");
    ...
    case GATT_COMM_EVENT_CLIENT_STATE:
        ...
        __ble_state_to_user(packet[0], little_endian_read_16(packet, 0));
        break;
...
static void dg_central_init(void)
{
    ...
    if (!dg_bond_device_table) {
        ...
        dg_bond_device_table = __bt_malloc(table_size);
...
static void dg_timer_handle_reset_scan(void)
{
    //dongle上电后回连已配对设备，根据地址创建连接，若此时被连接设备未在广播，则创建连接失败，定时3.5s重新打开scan
    u8 info = 1;
    dg_timer_handle(&info);
}
static u8 tmp_sys = 0;
...
#endif
        tmp_sys = 1;
        dg_timer_id = sys_timeout_add(&tmp_sys, dg_timer_handle, POWER_ON_SWITCH_TIME);
    } else {
        ble_gatt_client_scan_enable(1);
        log_info("just scan");
    }
...
void bt_ble_exit(void)
{
    ...
    if (dg_bond_device_table) {
        extern void __bt_free(void *p);
        __bt_free(dg_bond_device_table);
...
{
    int ret = 0;
    ret = ble_dongle_send_data_ext(data, len);
    return ret;
}
void app_ble_recv_callback_register(int (*callback_func)(u8 *, u16))
{
    local_irq_disable();
    dongle_audio_input_handler = callback_func;
    local_irq_enable();
}
#endif
