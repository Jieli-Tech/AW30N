...
void sm_set_master_reconn_encryption(void *p);
void sm_set_master_request_pair(void *p);
 ...
 */
/*************************************************************************************************/
void dongle_return_online_list(void *p)
{
    //返回所有gatt_client设备
...
void check_is_reconn_succ(void *p)
{
    ...
    if (is_reconn_address_device) {
        ...
        if (state) {
            ...
            temp_st = 1;
            sys_timeout_add(&temp_st, sm_set_master_reconn_encryption, 1000);//关闭加密失能
            sys_timeout_add(&temp_st, sm_set_master_request_pair, 1000);//关闭加密失能
        ...
        if (state == 0) {
            ...
            is_reconn_address_device = 0;
            u8 enable = 1;
            sm_set_master_reconn_encryption(&enable);
            sm_set_master_request_pair(&enable);
        }
    } else {
...
static void dongle_return_online_device_massage(void)
{
    ...
    printf("device_map_len %d", byte_number);
    put_buf((const u8 *)online_device, byte_number);
        ...
        if (buf_total[7 + packet_length - 1] == 0x00) {
            ...
            if (reconn_timer == 0) {
                ...
                if (0 == ble_gatt_client_create_connection_request(&buf_total[7 + 1], 0x00, 0)) {
                    ...
                    reconn_timer = sys_timeout_add(NULL, check_is_reconn_succ, 8000);//暂时设置重连时间为8s
