...
void cfg_file_parse(u8 idx)
{
    ...
    if (ret == (sizeof(KEY_OP) * KEY_NUM_MAX)) {
    ...
    if (ret == sizeof(AUTO_LOWPOWER_V_CONFIG)) {
    ...
    if (ret == sizeof(AUTO_OFF_TIME_CONFIG)) {
    ...
    if (ret == sizeof(LRC_CONFIG)) {
        log_info("lrc cfg:");
        log_info_hexdump((const u8 *)&lrc_cfg, sizeof(LRC_CONFIG));
